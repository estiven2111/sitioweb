<?php
 ...
 if($total_diario == false){
    ...
    $te= "INSERT INTO registro VALUES (NULL,'".date('Y-m-d')."','".$total_diario1[0]['Diario']."','','';";
    var_dump($te);
   
 $sentenciaSQL = $conexion->prepare("INSERT INTO registro VALUES (NULL,'".date('Y-m-d')."','".$total_diario1[0]['Diario']."','','';");
 $sentenciaSQL->execute();
 }else{
     $sentenciaSQL = $conexion->prepare("SELECT SUM(total) as Diario from pedidos where Fecha_Pedido = '".date('Y-m-d')."';");
     $sentenciaSQL->execute();
     $total_diario = $sentenciaSQL->fetchAll(PDO::FETCH_ASSOC);
     $sentenciaSQL = $conexion->prepare("UPDATE registro SET Diario = '".$total_diario."' WHERE fecha = '".date('Y-m-d')."';");
 }
